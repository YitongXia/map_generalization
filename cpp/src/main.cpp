...
int main(int argc, const char * argv[]) {
    ...
    else if(ifs.is_open()==1)
    {
        ...
        // create unordered edges using ordered edges
        ...
        Triangulation::output_obj(vertex_list,triangulation_list,output_path);
        std::cout<<"output obj file completed"<<std::endl;

        std::cout<<"output csv file now.."<<std::endl;
        WriteCSV::output_dart(darts_list,output_path);
        WriteCSV::output_edge(unordered_edges,output_path);
        WriteCSV::output_vertex(vertices,output_path);
        WriteCSV::output_faces(faces,output_path);
        WriteCSV::output_volume(volumes,output_path);
